...
exports.elevateCommand = (command, options) => {
  ...
  // have changed every time we upgrade `sudo-prompt`.
    ...
    return _.includes(_.trim(error.message), 'is not in the sudoers file');
  }, () => {
    throw errors.createUserError({
      title: 'Your user doesn\'t have enough permissions to proceed',
      description: 'This application requires sudo permissions to be able to write to drives'
    });
  }).catch({
